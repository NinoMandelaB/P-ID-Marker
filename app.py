...
with st.sidebar:
    ...
    if all_pdfs:
        ...
        selected_pdf_bytes = cur.fetchone()[0]
    else:
...
with col1:
    ...
    # Get existing elements and overlays for this PDF/page
    ...
    if canvas_result.json_data and 'objects' in canvas_result.json_data:
        ...
        if new_rects:
            st.info("Rectangle drawn! Fill out below to link with a new element.")
            with st.form("add_element_for_annotation", clear_on_submit=True):
                ...
                if submit:
                    rect = new_rects[-1]  # last drawn, most recent
                    ...
                    cur2.execute(
                        ...
                        (pid_doc_id, type, serial_number, position, internal_number,
                         ...
                         overlay_page, overlay_x, overlay_y)
                        ...
                        (
                            ...
                            att_bytes, att_name,
                            page_no, rect["left"], rect["top"]
                        )
                    )
                    conn.commit()
                    st.success("Element created and linked to annotation box. Refresh to see it on canvas.")

# ----------- ELEMENT TABLE WITH HIGHLIGHT AND FILTERS -----------------
with col2:
    ...
    # Highlight on canvas if table row is clicked
    if selected_rows:
        sel_id = df.loc[selected_rows[0]]["ID"]
        sel_el = [el for el in elements if el['id'] == sel_id][0]
        st.info(
            f"Element {sel_el['type']} @ Page {sel_el['overlay_page']+1} "
            f"({sel_el['overlay_x']:.0f}, {sel_el['overlay_y']:.0f})"
        )
        # Show link, image, and attachment download
        if sel_el['photo']:
...
# --------- BIDIRECTIONAL: CANVAS CLICK (SHOW EL DETAILS) --------------
if canvas_result.json_data and 'objects' in canvas_result.json_data:
    ...
    if (
        ...
        if eid:
            sel = [el for el in elements if el['id'] == eid]
            if sel:
